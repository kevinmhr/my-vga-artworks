    ...
    //       draw_sprite(surface,levelbg3,bgscrolx/2,bgscroly);
         ...
         int  horizontal=20-add;
              int    vertical=-20;
               ...
               int ui=0;
                for(int k=0;k<160;k++){
                  xi+=20; if (xi>=320){yi+=20;xi=0; }


                //  blit(sprsheet,surface2,0,50,(xi)-(add)+0,yi,10,10);
                 rect(surface2,(xi)-(add)+0,yi,(xi)-(add)+0+20,yi+20,7);
             //  hline(surface2,(xi)-(add)+0,yi,(xi)-(add)+0+20,230);
              //   putpixel(surface2,(xi)-(add)+0,yi,0);
       ...
       int tempvert;
           ...
           for (int k=0;k<(20000);k++){
            ...
            maplenght=(575)+30;
                 vertical+=1;
          if (vertical>(maplenght)){horizontal=horizontal+20;vertical=0;            }
         //   if(horizontal>320){horizontal=0;}
...
tuu++;
                   ...
                   if (maptemp[k]==1055) {
                     masked_blit(levelbg1,surface2,0,150,(i*10)-add,vertical-20,100,80);
                    }


     if (maptemp[k]==1054) {
                     masked_blit(levelbg1,surface2,80,100,(i*10)-add,vertical-20,60,40);
                    }
...
int js=0;
      ...
      masked_blit(walkingspr2,surface2,frx,14,(i*10)-add,vertical,12,9);
  ...
  */
          ...
          if(maptemp[k]==2060)    {y=66;x1=19;}
                          tempvert=vertical;
//       masked_blit( sprsheet,surface2,x1-10,y-10,((horizontal)),(vertical*20)-add2 ,20, 20);
       ...
       /*
          ...
          masked_blit(levelbg1,surface2,220,145,(i*10)-add,vertical-20,20,20);
           rectfill(collisionpad,(i*10)-add+10,vertical-20,(i*10)-add+25,vertical,4);
                   ...
                   if (maptemp[k]==1800) {
          //  levelbg3=create_bitmap(60,30);
          masked_blit(levelbg1,surface2,180,150,(i*10)-add,vertical-20,20,20);
      ...
      masked_blit(levelbg1,surface2,49,89,(i*10)-add,vertical-35,30,60);
    ...
    if (maptemp[k]==1801) {
          ...
          masked_blit(levelbg1,surface2,2,80,((((i-1)*10))-add)*2,vertical-20,40,25);
...
//              putpixel(surface,(x/2)+spider.x[enemicnt]-1-(z*10)-add,(y/2)+spider.y[enemicnt]-(zv*10)-add2+1,0);
             ...
             putpixel(surface,x3+spider.x[enemicnt]-1-(z*10)-add,y3+spider.y[enemicnt]-(zv*10)-add2,colx+spider.color[enemicnt]);
             ...
             //       masked_blit(walkingspr2,surface,155+spidanimx+x3+3,261+y3,spider.x[enemicnt]-1-(z*10)-add,spider.y[enemicnt]-(zv*10)-add2,10,10);
              ...
              //      masked_blit(walkingspr2,surface,155+spidanimx+x3+3,261+y3,spider.x[enemicnt]-1-(z*10)-add,spider.y[enemicnt]-(zv*10)-add2,10,10);
                   ...
                   rectfill(collisionpad,spider.x[enemicnt]-(z*10)-add-5,spider.y[enemicnt]-15-add2-(zv*10),spider.x[enemicnt]-(z*10)-add+15,spider.y[enemicnt]-15-add2-(zv*10)+20,90);
   ...
   //         draw_sprite(surface,sprite,);
         ...
         if (colx==90){ hits+=1;   play_midi(grand,0); midi_seek(250);      k34=1;
           ...
           /*
           colx=getpixel(collisionpad,bulletx[u],bullety[u]-zvb-10);
         if (colx==90){
...
//        textprintf_ex(surface,font,160,0,250,0,"%i",plus);
...
//          if (key[KEY_DOWN]) {plus+=t; frame=frame+400;   if (frame>800){frame=0; } }
  ...
  //  if (plus>9400){plus==0;}
      ...
      if (key[KEY_DOWN]) {   releasel=0; faced=-1;     playery=playery-t2;     framey=172; slow++; if (slow>10){slow=0;        framex+=16;          }
        ...
        if (framex>=201){  framex=155;}
           ...
           //              if (z>8000){z=0;}
    if (key[KEY_UP]) {    releaser=0; faced=1;     playery=playery+t;      framey=230;
           ...
           if (timerb<0){timerb=0;}
            ...
            // timerb=0;
               ...
               //     bulletcnt++;
                   if (key[KEY_LEFT]) { playerx=playerx-1;}
      if (key[KEY_RIGHT]) {   playerx=playerx+1;}


                                           if (playerx>=151) {
                                        add++; if (add>20){add=1;  plus+=maplenght+1;z++;  } }
                                    if (playerx<=130) {
                              add--;bgaddh++;  if (add<1)   {add=20;     plus-=maplenght+1;z--;   } }


                           if (playery<70){ zvb--;    add2++;  if (add2>20){ add2=0;    plus+=1;zv=zv-1;}}
                            playery+=2;
                           if (playery>80){ zvb--;   add2-=3;  if (add2<0){ add2=20;    plus-=1;zv=zv-1;} }
                  ...
                  for (int ib=0;ib<100;ib+=1){
                    ...
                    //   if(timerb>300){timerb=300;}
                            ...
                            //    rect(screen,bulletx[i],bullety[i]-(zvb),bulletx[i]+thickx+1,bullety[i]-(zvb)+thicky+1,2);
                                  ...
                                  colx=getpixel(collisionpad,bulletx[ib],bullety[ib]-zvb);
                                      if(colx==4){ bulletx[ib]=400;  bullety[ib]=400;}
                                        colx=getpixel(collisionpad,bulletx[ib],bullety[ib]-zvb);
                   ...
                   //    bulletx[bulletcnt]=playerx;
                                if(bulletx[bulletcnt]>320){bulletx[bulletcnt]=400;bullety[bulletcnt]=400;  }
                                 if(bulletx[bulletcnt]<0){bulletx[bulletcnt]=400;bullety[bulletcnt]=400;  }
                                   if(bullety[bulletcnt]>=240){bulletx[bulletcnt]=400;bullety[bulletcnt]=400; }
...
/*       if (rotateang<1){rotateang=(sintab2[(plus2-100)]/3)+0.8;  rotatex=(cos(rotateang)+sin(rotateang));
  ...
  */
  ...
  for (int z=0;z<512;z+=1){
   ...
   //     if (key[KEY_DOWN]){playery-=0.001;}
           /*
        if (key[KEY_UP]){ rotateangi+=0.00001;
                           }
        if (key[KEY_DOWN]){ rotateangi-=0.00001;
  ...
  if (key[KEY_RIGHT]){  rotateang+=0.00001;
 ...
 if (key[KEY_LEFT]){  rotateang+=-0.00001;
 ...
 /*
          for (int i=0;i<100;i++){
                     ...
                     //   cb.i[i]+=cb.rotatex[k]/100;
                     //   cb.j[i]+=cb.rotatey[k]/100;
                    //    cb.l[i]+=(sin(k)-cos(k));
   ...
   cb.rotatex[z]=(((cb.i[z]*dis))*(cos(rotateang)))+(((cb.l[z]*dis)*sin(rotateang)));
   cb.rotatey[z]=((cb.j[z]*dis));
   cb.rotatel[z]=(((cb.i[z]*dis))*(-sin(rotateang)))+(((cb.l[z]*dis)*cos(rotateang)));
       ...
       //  l2[k]=(l[k]);
             ...
             //          i[k]=(int)((i[k])+k);
                 ...
                 cb.j2[z]=(cb.rotatey[z]*cos(rotateangi))-(cb.rotatel[z]*sin(rotateangi));
                 cb.l2[z]=(cb.rotatey[z]*sin(rotateangi))+(cb.rotatel[z]*cos(rotateangi));

                 
               

                      cb.rotatey[z]=cb.j2[z];
                       cb.rotatel[z]=cb.l2[z];
           ...
           //         l2[k]=l2[k]+(j2[k]*(rotatex[k]))/10;
            ...
            //       j2[k]=j2[k]*(playery/10);
            //     i2[k]=i2[k]*(playery/10);
              //   j2[k]=j2[k]*(z/10)/5;
          ...
          //    col=rotateang*10;
              ...
              cb.projx[z]=((cb.rotatex[z]*20)/(cb.rotatel[z]/5))+160;
              cb.projy[z]=((cb.rotatey[z]*20)/(cb.rotatel[z]/5))+100;
  ...
  //   putpixel(surface,cb.projx[0],cb.projy[0],250);
   //  putpixel(surface,cb.projx[1],cb.projy[1],250);
    // putpixel(surface,cb.projx[2],cb.projy[2],250);
    // putpixel(surface,cb.projx[3],cb.projy[3],250);
   putpixel(surface,((cb.projx[z])),((cb.projy[z])),255);

//circlefill(surface,((cb.projx[k])),((cb.projy[k])),1,col+100);
// putpixel(surface,((cb.projx[k])),((cb.projy[k])),col+100);
 if (k<80){
//    line(surface,cb.projx[z+k],cb.projy[z],cb.projx[z+k],cb.projy[z],cb.col[z]);


  /*
   line(surface,cb.projx[0],cb.projy[0],cb.projx[4],cb.projy[4],cb.col[k]);
   line(surface,cb.projx[1],cb.projy[1],cb.projx[5],cb.projy[5],cb.col[k]);
   line(surface,cb.projx[2],cb.projy[2],cb.projx[6],cb.projy[6],cb.col[k]);
   line(surface,cb.projx[3],cb.projy[3],cb.projx[7],cb.projy[7],cb.col[k]);


    line(surface,cb.projx[0],cb.projy[0],cb.projx[2],cb.projy[2],cb.col[k]);
   line(surface,cb.projx[1],cb.projy[1],cb.projx[0],cb.projy[0],cb.col[k]);
   line(surface,cb.projx[1],cb.projy[1],cb.projx[3],cb.projy[3],cb.col[k]);
   line(surface,cb.projx[2],cb.projy[2],cb.projx[3],cb.projy[3],cb.col[k]);

 
    line(surface,cb.projx[4],cb.projy[4],cb.projx[6],cb.projy[6],cb.col[k]);
   line(surface,cb.projx[5],cb.projy[5],cb.projx[4],cb.projy[4],cb.col[k]);
   line(surface,cb.projx[5],cb.projy[5],cb.projx[7],cb.projy[7],cb.col[k]);
  line(surface,cb.projx[7],cb.projy[7],cb.projx[6],cb.projy[6],cb.col[k]);
    */
 }
                       }
...
//    enemii.x=1;
 ...
 collisionpad=create_bitmap(320,240);
  collisionpad2=create_bitmap(3200,640);
   ...
   grand=load_midi("music.mid");
     ...
     surface=create_bitmap(320,240);
       surface2=create_bitmap(320,240);
