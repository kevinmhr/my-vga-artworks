...
// tuu=0; }   fry=77;
 ...
 tuy++;
   fry=77;
        ...
        eniy[enemicnt]=sin(enix[enemicnt]/20+plus/20)*40;
...
// clear_to_color(sprites,4);
          ...
          masked_blit(walkingspr,surface,frx,fry,enix[enemicnt]+(i*10)-add,vertical-add2+eniy[enemicnt],20,20);




      rectfill(collisionpad,enix[enemicnt]+((i-1)*10),vertical-add2+eniy[enemicnt],enix[enemicnt]+((i+1)*10),vertical+20-add2+eniy[enemicnt],90);
//         rectfill(surface,enix[enemicnt]+((i-1)*10),vertical-add2+eniy[enemicnt],enix[enemicnt]+((i+1)*10),vertical+20-add2+eniy[enemicnt],90);
...
//          if (key[KEY_DOWN]) {plus+=t; frame=frame+400;   if (frame>800){frame=0; } }
  ...
  //  if (plus>9400){plus==0;}
   ...
   //     if (playerx>110) {   playerx-=1;      if (bgscrolx>400){bgscrolx=0;}  bgscrolx=bgscrolx+(double)tx;  add++; if (add>7){add=0;  plus+=1;  z++;   }  }
   //     if (playerx<90) {             playerx+=1; if (bgscrolx<0){bgscrolx=400;} bgscrolx=bgscrolx-(double)tx2;     add--;  if (add<0)   {add=7;  plus-=1;  z--; } }

    add++; if (add>9){add=0;  plus+=1;     }
           ...
           bullethit=0  ;
                     ...
                     if(bulletx[bulletcnt]<0){timerb=0;} 
                    ...
                    for (int i=0;i<timerb;i++){
                             ...
                             bulletcnt=i+1;
                              ...
                              bulletx[bulletcnt]=bulletx[bulletcnt]+1;
