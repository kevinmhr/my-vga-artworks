...
if (maptemp[k]==1077){ numtogo++; spider.x[k]=(i*5);   spider.bdiry[k]=0;  spider.bdirx[k]=0;
spider.y[k]=y-60;
spider.health[k]=60;
...
spr.x[k]=0;
if (maptemp[k]==1078){spr.x[k]=(i*5);   spr.bdiry[k]=0;  spr.bdirx[k]=0;
spr.y[k]=y-60;
   ...
   //       if(maptemp[k]==2060)    {y=75;x1=99;}
       ...
       masked_blit( sprsheet,surface,x1-10,y-10,((i*5))-add,vertical-add2,5, 5);
                 rectfill(collisionpad,(i*5)-add,vertical-add2,((i*5)-add)+5,vertical+7-add2,4);
                     ...
                     if(colx==4){ maptemp[k]=0;bulletx[lastbullet]=800;   }
...
for (int i=0;i<80000;i++){
     ...
     //         if (colr==1){if (colu==1){ sleeps.dirx[ballcnt]=-1; sleeps.diry[ballcnt]=0;          }}
      //        if (coll==1){if (colu==1){ sleeps.dirx[ballcnt]=1; sleeps.diry[ballcnt]=0;          }}
               ...
               if (sleeps.diry[ballcnt]==1){   sleeps.y[ballcnt]+=spdy;  i=2; j=5;  }
               if (sleeps.diry[ballcnt]==-1){   sleeps.y[ballcnt]-=spdy; i=2; j=5;  }
                 if (sleeps.dirx[ballcnt]==1){ sleeps.x[ballcnt]+=spdx; j=2; i=5;   }
               if (sleeps.dirx[ballcnt]==-1){  sleeps.x[ballcnt]-=spdx; j=2; i=5;    }
...
//              masked_blit(walkingspr2,surface,269+ballframex,ballframey,ballx,bally-(zv*20),16,18);
                       rectfill(surface, ballx,bally-(zv*5),ballx+i,bally-(zv*5)+j,3);
               ...
               if (sleeps.dirx[ballcnt]==-1){ putpixel(surface, ballx,bally-(zv*5)+2,238);putpixel(surface, ballx,bally-(zv*5),238);  }
                  ...
                  rectfill(collisionpad,ballx-10,bally-(zv*5)-10,ballx+10,bally-(zv*5)+10,90);
         ...
         hits+=1;   play_midi(grand,0); midi_seek(250);     k34=1;
            bullettrig=0;    explosx=bulletx[lastbullet];     explosy=bullety[lastbullet]-zvb;


                          if (k34==1){      explos.timer=0;

           bulletspd=2; midiseek=0; k34=0; /* maptemp[ballcnt]=0;*/ bulletx[lastbullet]=1400;      mapfunc();
                            }
      ...
      //    if (t2player==0){accely2=0;    }
       ...
       //         playerx=(playerx)+(((((playerx)*rotatex)+((playery)*rotatey))))/1000;
        //           playery=(playery)+(((((playerx)*rotatex)-((playery)*rotatey))))/1000;
           ...
           if (playerx>250) {
                                          playerx-=1;    add+=1; if (add>5){add=0;  plus=(int)plus+1;z++;  } }
                                    if (playerx<150) {
                           playerx+=1;   add-=1;bgaddh++;  if (add<0)   {add=5;     plus=(int)plus-1;z--;   } }
                    if (playery>180){ zvb++; playery-=1;                       add2+=1;  if (add2>5){ add2=0;   plus+=maplenght+1;zv=zv+1; }  }
                    if (playery<110){ zvb--; playery+=1;    add2-=1;  if (add2<0){ add2=5;    plus-=maplenght+1;zv=zv-1;}  }
         ...
         if (key[41]) {                  angcor-=0.03;

                               if (collided==0){   ang-=0.03; }
        ...
        if (framex>175){  framex=155;}
           ...
           //              if (z>8000){z=0;}
    if (key[43]) {   releaser=0; faced=1;  if (collided==0){   ang+=0.03; }    angcor+=0.03;
      framey=230;
                 ...
                 slow++; if (slow>10){slow=0;    framex+=16;
                  ...
                  if (key[45]){            faced=2;        accelx+=0.008;    accely+=0.2;    accely2+=0.2;  accelx2+=0.2;
         ...
         if (key[42]){       faced=-2;                 accelx-=0.008;     accely-=0.2;  accely2-=0.2;  accelx2-=0.2;
           ...
           bullethit=0  ;
                  ...
                  for (int ib=0;ib<100;ib+=1){
                    ...
                    //   if(timerb>300){timerb=300;}
                                 ...
                                 bulletx[ib]= bulletx[ib]+(30*((sin(angcor)+cos(angcor))/10))+warpx;

                                 bullety[ib]= bullety[ib]+(30*((-sin(angcor)+cos(angcor))/10)+warpy);

                            //    rect(screen,bulletx[i],bullety[i]-(zvb),bulletx[i]+thickx+1,bullety[i]-(zvb)+thicky+1,2);
                                         rectfill(surface,bulletx[ib],bullety[ib]-zvb,bulletx[ib]+1,bullety[ib]+1-zvb,238);
