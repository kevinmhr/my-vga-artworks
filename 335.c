           ...
           //   if (z>32){z=0;entities();}
            ...
            //         if (sleeps.timer>=3000) { sleeps.dirx[ballcnt]=0;sleeps.diry[ballcnt]=-1;      }
                        ...
                        //       bty[ballcnt]=0;
                                        ...
                                        //  sleeps.diry[ballcnt]=0;
                                         //                            sleeps.dirx[ballcnt]=0;
                                                 ...
                                                 if  ( sleeps.x[ballcnt]>320 ){  sleeps.x[ballcnt]=160;     }
                                                 if  ( sleeps.x[ballcnt]<0 ){  sleeps.x[ballcnt]=160;       }
                                                   if  ( sleeps.y[ballcnt]>200 ){  sleeps.y[ballcnt]=100;     }
                                                     if  ( sleeps.y[ballcnt]<0 ){  sleeps.y[ballcnt]=100;      }




                                     if (sleeps.timer>2000) {
              ...
              int  bally=sleeps.y[ballcnt]-add2;
                                                      ...
                                                      if(   sleeps.colyw!=0 ){
                                if (playery>(sleeps.y[ballcnt]-(zv*20))){
                                                                             if (playery-(sleeps.y[ballcnt]-(zv*20))<rt){   sleeps.dirx[ballcnt]=0;
                                                          circlefill(surface, sleeps.x[t], sleeps.y[t],0,237);
                                         ...
                                         if ((sleeps.y[ballcnt]-(zv*20))-playery<rt){   sleeps.dirx[ballcnt]=0;
                                                     circlefill(surface, sleeps.x[t], sleeps.y[t],0,237);
                                         ...
                                         if (playerx-(sleeps.x[ballcnt]-(z*20))<rt){   sleeps.diry[ballcnt]=0;
                                                  circlefill(surface, sleeps.x[t], sleeps.y[t],0,237);
                                         } }

                                                         if (playerx<(sleeps.x[ballcnt]-(z*20))){
                                    
                                         if ((sleeps.x[ballcnt]-(z*20))-playerx<rt){   sleeps.diry[ballcnt]=0;
                                              circlefill(surface, sleeps.x[t], sleeps.y[t],0,237);

                                         } }
                                                                         }

                                                                /*      if (sleeps.timer2>2000){   sleeps.timer2=0;
                                             sleeps.dirx[ballcnt]=0;
                                             sleeps.diry[ballcnt]=0; }  */
             ...
             /*
               ...
               */
               ...
               if (sleeps.diry[ballcnt]==1){    sleeps.y[ballcnt]+=0.03; }
               if (sleeps.diry[ballcnt]==-1){   sleeps.y[ballcnt]-=0.03;  }
                 if (sleeps.dirx[ballcnt]==1){ sleeps.x[ballcnt]+=0.03; }
               if (sleeps.dirx[ballcnt]==-1){  sleeps.x[ballcnt]-=0.03;  }
   

                ballx=sleeps.x[ballcnt]-add;
...
//              masked_blit(walkingspr2,surface,269+ballframex,ballframey,ballx,bally-(zv*20),16,18);
          ...
          //             rectfill(surface, ballx,bally-(zv*20),ballx,bally-(zv*20),3);
               ...
               /*              int colx=getpixel(collisionpad,playerx,playery);
                  if (colx==16) {init();}  */
                                       ...
                                       /*
                                                 if  ( sleeps.x[t]<160 ){
                                              sleeps.x[t]-=(-sleeps.x[t])/200 ;    }
                                         */
                                                /*
                                                   if  ( sleeps.y[t]-100>100 ){
                                              sleeps.y[t]+=(sleeps.y[t])/500 ;
                                                              }

                                                 if  ( sleeps.y[t]<100 ){
                                              sleeps.y[t]+=(-sleeps.y[t])/200 ;
                                                              }
                                                  */
                                  ...
                                  for (int t=0;t<1000;t++){
                                        t2++;
                                              ...
                                              //    hline(surface,0,100,320,0);
                                               //   vline(surface,160,0,200,0);



                                              //    textprintf_ex(surface,font,10,10,250,0,"%f",movex);
                                              ...
                                              sleeps.x[t]+=(sin((sleeps.x[t]-160)/100)*4);
                                              sleeps.y[t]+=(sin((sleeps.y[t]-100)/100)*4);
                                                      if ( sleeps.y[t]!=100){      if ( sleeps.x[t]!=160){
                                                      circlefill(surface, sleeps.x[t], sleeps.y[t],1,3);
                                                                             }                       }
                                              ...
                                              if (t2==2){
                                              sleeps.x[t]+=(sin((sleeps.x[t]-160)/100));
                                              sleeps.y[t]+=(sin((sleeps.y[t]-100)/100));
                                                        if ( sleeps.y[t]!=100){      if ( sleeps.x[t]!=160){   circlefill(surface, sleeps.x[t], sleeps.y[t],0,3);
                                                                                        }            }   }


                                                 if (t2>4){ t2=0;
                                              sleeps.x[t]+=(sin((sleeps.x[t]-160)/100)/2);
                                              sleeps.y[t]+=(sin((sleeps.y[t]-100)/100)/2);
                                                        if ( sleeps.y[t]!=100){      if ( sleeps.x[t]!=160){
                                                    circlefill(surface, sleeps.x[t], sleeps.y[t],0,3); } }
                                                                                          }
...
//          if (key[KEY_DOWN]) {plus+=t; frame=frame+400;   if (frame>800){frame=0; } }
  ...
  //  if (plus>9400){plus==0;}
         ...
         faced=-1;    playerx=playerx-tx2*2;   movex+=1;  framey=172; slow++; if (slow>10){slow=0;        framex+=16;          }
        ...
        if (framex>175){  framex=155;}
           ...
           //              if (z>8000){z=0;}
    if (key[43]) {   releaser=0; faced=1;     playerx=playerx+tx*2; movex-=1;     framey=230;
         ...
         framey=210;   slow++; if (slow>10){slow=0;        framex+=16;          }  playery=playery+t2player*2;   movey+=1;                     if (framex>175){framex=155;}
      }
         ...
         framey=191;  slow++; if (slow>10){slow=0;        framex+=16;          }    playery=playery+tplayer*2;   movey-=1;                    if (framex>175){framex=155;}
     }
                                                                     bullethit=1;
